...
EQUIP_BLUE_BULL_CUSTOM = GenerateNewEquipmentID and GenerateNewEquipmentID() or 16

local bluebull = {
	id = EQUIP_BLUE_BULL_CUSTOM,
	loadout = false,
	type = "item_passive",
...
hook.Add("SetupMove", "MultiJump", function(ply, mv)
	if ply:HasEquipmentItem(EQUIP_BLUE_BULL_CUSTOM) then
...
hook.Add("TTTPlayerSpeedModifier", "BlueBullSpeed" , function(ply)
	if ply:HasEquipmentItem(EQUIP_BLUE_BULL_CUSTOM) 
    and not ply:GetNWBool("DRDead") 
    and not ply:GetActiveWeapon() == "weapon_ttt_homebat" 
    and not ply:GetNWBool("ItsHighNoon") 
    and not ply:GetNWBool("ItsHighNoonshooting") 
    and not ply.RandomatSuperSpeed 
    and not ply.RandomatSpeed 
    then
...
if SERVER then
	hook.Add("TTTOrderedEquipment", "TTTBlueBull3", function(ply, equipment, is_item)
		 if is_item and equipment == EQUIP_BLUE_BULL_CUSTOM then
	...
	hook.Add("EntityTakeDamage", "BlueBullFallDamage", function(ent, dmg)
		if IsValid(ent) and ent:IsPlayer() and ent:HasEquipmentItem(EQUIP_BLUE_BULL_CUSTOM) and dmg:IsFallDamage() then
    ...
    hook.Add("TTTBoughtItem", "TTTBlueBull2", function()
        if LocalPlayer():HasEquipmentItem(EQUIP_BLUE_BULL_CUSTOM) then
            yCoordinate = getYCoordinate(EQUIP_BLUE_BULL_CUSTOM)
        end
    end)
    ...
    hook.Add("TTTBodySearchEquipment", "BlueBullCorpseIcon", function(search, eq)
        search.eq_bluebull = util.BitSet(eq, EQUIP_BLUE_BULL_CUSTOM)
    end)

    hook.Add("TTTBodySearchPopulate", "BlueBullCorpseIcon", function(search, raw)
        if not raw.eq_bluebull then
            return 
        end
